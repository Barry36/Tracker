...
{
  // Create an array of objects (arrayOfChangedTrigger) to store trigger info when edit triggers
  // 
...
$('.saveTriggerRow').on('click',function(e){
      ...
      // Check if courseid set by the user
      ...
      // Update Trigger (including conditions update)
        ...
        $('#inner-table_'+TriggerId).find('select.condition-param').each(function(){
          ...
          if (tmp != 'coursename') {
            var targetCourseIdVal = $(this).closest('tr').find('input:text.condition-value').val();
            var targetCourseIdOperator = $(this).closest('tr').find('select.condition-operator').val();
            if(!$.isNumeric( targetCourseIdVal )){
              errorExist = 1;
              alert(tmp + ' value needs to be a number!');
              HighlightConditionError($(this));
              return false;
            }else{
              if (tmp == 'courseid' && parseInt(targetCourseIdVal) <= 0 && targetCourseIdOperator == 'Less_than') {
                errorExist = 1;
                alert('courseid needs to be a positive integer!');
                HighlightConditionError($(this));
                return false;
              }
              $(this).closest('tr').removeClass('conditio-value-invalid-tr');
              $(this).closest('tr').find('input:text.condition-value').removeClass('conditio-value-invalid-input-text');
            }
          }
        });
        ...
        // Check if courseid set by the user, 1 means courseid was set, -1 means courseid was not set
        if(jQuery.inArray("courseid", ParamArray2) !== -1){
          var courseid = "1";
        ...
        $('#viewcourseidbtn_' + TriggerId).addClass('hide');
    ...
    //     // Update all the conditions of this trigger 
    ...
    //           type     : "POST",
    //           url      : "update.php", 
    //           data     : {myTriggerInfo:jsonTriggerInfo,courseid:courseid},
    //           success: function(data){
    //             console.log(data);
    //         }
    //     });
...
$('.EditTrigger').on('click',function(){
  ...
  $('#viewcourseidbtn_' + currentTriggerId).removeClass('hide');
...
// *********************************************************************************************************************************
// ***************                        This section has all error checking fucntions                              ***************
// *********************************************************************************************************************************
  function HighlightConditionError(obj){
    obj.closest('tr').addClass('conditio-value-invalid-tr');
    obj.closest('tr').find('input:text.condition-value').addClass('conditio-value-invalid-input-text');
  }
});
